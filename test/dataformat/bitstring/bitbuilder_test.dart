...
import 'package:tonutils/tonutils.dart' show BitBuilder, InternalAddress;

void main() {
  group('dataformat/bitstring/bitbuilder', () {
    test('Serializing uint', () {
      var cases = <List>[
        [10290, 29, '00014194_'],
        [41732, 27, '0014609_'],
        [62757, 22, '03D496_'],
      ...
      for (var c in cases) {
        ...
        builder.writeUint(BigInt.from(c[0]), c[1]);
        var bits = builder.build();

        expect(bits.toString(), c[2]);
      }
    });
    ...
    test('Serializing int', () {
      ...
      for (var c in cases) {
        var builder = BitBuilder();
        builder.writeInt(BigInt.from(c[0]), c[1]);
        var bits = builder.build();
    ...
    test('Serializing coins', () {
      ...
      for (var c in cases) {
        ...
        expect(bits.toString(), c[1]);
      }
    });

    test('Serializing addresses', () {
      var cases = <List>[
        [
      ...
      for (var c in cases) {
        var builder = BitBuilder();
        builder.writeAddress(InternalAddress.parse(c[0]));
        var bits = builder.build();
